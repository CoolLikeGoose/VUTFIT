...
void doOperation( Stack *stack, char c, char *postfixExpression, unsigned *postfixExpressionLength ) {
	if (Stack_IsEmpty(stack)) { 				//		If stack is empty we can just push operation
		Stack_Push(stack, c);
		return;
	}
...
void expr_value_push( Stack *stack, int value ) {
    ...
    for (int i = sizeof(int) - 1; i >= 0; i--)							//		Int is 4 byte variable so just push to stack 4 times
    {																	//		dived number in 4 bytes
...
void expr_value_pop( Stack *stack, int *value ) {
	*value = 0;
    ...
    {
		Stack_Top(stack, &byte);										//		Just take from top of the array 4 top bytes 
		Stack_Pop(stack);	
        *value |= (byte & 0xFF) << (i * 8);								//		And recreate the origin number with or operation 
    }
}


//Help funtion for eval
...
int provideOperation(char operation, Stack *stack) {
	...
	return -99;			//Unreacheble (probably)
}

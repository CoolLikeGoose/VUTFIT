...
do
	test="${src%.*}"
	echo "$(basename "$src" .src)" | tee -a >(cat >> "$LOGFILE")

	"$PYTHON" -O "$PARSE" < "$src" > "$test".your_out 2> /dev/null
	echo -n "$?" > "$test".your_rc | tee -a >(cat >> "$LOGFILE")

    PASS=1

    if [[ $(diff -bw "$test".rc "$test".your_rc) ]]; then
        ERROR "Exit code differs, expected $(cat "$test".rc), got $(cat "$test".your_rc)" | tee -a >(cat >> "$LOGFILE")
        PASS=0
    fi
    ...
    if [[ $PASS -ne 0 ]] && [[ $(cat "$test".your_rc) == "0" ]] && [[ -n "$(cat "$test.your_out")" ]] && ! java -jar "$jexamxml"/jexamxml.jar "$test".out "$test".your_out "$test"_diff.xml  /D "$jexamxml"/options > /dev/null; then
        ERROR "Output differs, see "$test"_diff.xml" | tee -a >(cat >> "$LOGFILE")
        PASS=0
    fi

    if [[ $PASS -eq 1 ]]; then
        OK | tee -a >(cat >> "$LOGFILE")
        PASSED=$((PASSED+1))
    else
        FAILED=$((FAILED+1))
    fi

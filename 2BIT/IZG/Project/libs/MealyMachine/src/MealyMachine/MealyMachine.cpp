...
bool MealyMachine::parse(BasicUnit const* data, size_t size) {
  ...
  do {
    auto const& state_lo   = _states.at(_currentState);
    auto const& chooser_lo = std::get<CHOOSER>(state_lo);
    symbolSize          = chooser_lo->getSize();
    ...
    _currentSymbolSize = symbolSize;
    _dontMove          = false;
    if (!_nextState(state_lo)) return false;
    if (!_dontMove) {
      _readingPosition += symbolSize * sizeof(BasicUnit);

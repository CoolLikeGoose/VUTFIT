 ...
 */
void skFlag_VS(OutVertex&outVertex,InVertex const&inVertex,ShaderInterface const&){
 ...
 * @param uniforms uniform variables
 */
void skFlag_FS(OutFragment&outFragment,InFragment const&inFragment,ShaderInterface const&si){
  auto uv = inFragment.attributes[0].v2;
  southKoreanFlag(outFragment.gl_FragColor,inFragment.gl_FragCoord, ivec2(si.uniforms[0].v2));
}
...
Method::Method(GPUMemory&m,MethodConstructionData const*): ::Method(m){
  mem.programs[0].vertexShader   = skFlag_VS;
  mem.programs[0].fragmentShader = skFlag_FS;
...
void Method::onDraw(SceneParam const&){
  auto&frame = mem.framebuffers[mem.defaultFramebuffer];
  mem.uniforms[0].v2 = vec2(frame.width,frame.height);
...
EntryPoint main = [](){registerMethod<Method>("SKFlag");};

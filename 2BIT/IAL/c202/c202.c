...
void Stack_Init( Stack *stack ) {
	...
	stack->topIndex = -1;													// 		Set stack size to 0
	stack->array = malloc(STACK_SIZE * sizeof(char));						// 		Initialize array of stack
}
...
bool Stack_IsEmpty( const Stack *stack ) {
	return stack->topIndex == -1 ? 1 : 0;									//		Check Length of stack
}
...
bool Stack_IsFull( const Stack *stack ) {
	return stack->topIndex == STACK_SIZE-1 ? 1 : 0; 						//		Check Length of stack
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	*dataPtr = stack->array[stack->topIndex];								//		Just return last element
}
 ...
 * @param stack Ukazatel na inicializovanou strukturu zásobníku
 */
void Stack_Pop( Stack *stack ) {											//		If stack isn't empty then just decrease stack length
	if (!Stack_IsEmpty(stack)) stack->topIndex--;							//		because later we can just rewrite this element
}
...
void Stack_Push( Stack *stack, char data ) {
	...
	stack->topIndex++;														//		Increase stack size
	stack->array[stack->topIndex] = data;									//		Write data on top of the stack
}
...
void Stack_Dispose( Stack *stack ) {
	...
	free(stack->array);														//		Free all memory for array
	stack->array = NULL;													//		Delete pointer to the already non-existed array
	stack->topIndex = -1;													//		Decrease stack size to 0
}
